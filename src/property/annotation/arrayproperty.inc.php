...
#[Attribute]
class ArrayProperty extends \buffalokiwi\magicgraph\property\ArrayProperty
{
  public function __construct( string $name, string $clazz = '', array $defaultValue = [], 
    array $flags = ['noinsert,noupdate'], string $behaviorClass = '', string $flagsClass = SPropertyFlags::class )
  {
    $bc = ( !empty( $behaviorClass )) ? new $behaviorClass() : null;
    $fc = new $flagsClass( ...$flags );
    
    $builder = new ObjectPropertyBuilder( EPropertyType::TARRAY(), $fc, null );
